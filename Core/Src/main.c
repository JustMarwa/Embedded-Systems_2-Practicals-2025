...
void Handle_Mode_3(void)
{
    if (sparkle_state == 0) {
        ...
        if (sparkle_delay_counter == 0) {
            ...
            // Prepare list of LEDs to turn off
            int sparkle_led_count = 0;
            for (int i = 0; i < 8; i++) {
        ...
        if (turn_off_index < sparkle_led_count) {
            ...
            uint32_t timer_period_ms = fast_mode ? 500 : 1000;
            uint32_t turn_off_delay = delay_ms / timer_period_ms;
            if (turn_off_delay == 0) turn_off_delay = 1;

            if (turn_off_timer >= turn_off_delay) {
            ...
            // All LEDs turned off, reset for next cycle
            sparkle_state = 0;
            sparkle_delay_counter = 0;
        }
    }
